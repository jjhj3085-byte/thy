...
def cmd_start(message: types.Message) -> None:
    ...
    if is_new:
        user_tag = f"@{message.from_user.username}" if message.from_user.username else "بدون يوزر"
        admin.admin_log(bot, f"🆕 <b>مستخدم جديد:</b>\n👤 {message.from_user.first_name} ({user_tag})\n🆔 <code>{message.from_user.id}</code>")
